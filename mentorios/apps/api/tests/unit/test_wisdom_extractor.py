...
class TestFormatActions:
    ...
    def test_single_action(self):
        actions = [{"action_type": "strike", "start_time": 1.0, "end_time": 2.0, "intensity": 0.8}]
        result = _format_actions(actions)
        assert "strike" in result
        assert "1.0s-2.0s" in result

    def test_action_with_subtype(self):
        actions = [{"action_type": "strike", "action_subtype": "jab", "start_time": 1.0, "end_time": 1.5}]
        result = _format_actions(actions)
...
class TestExtractWisdomFromScene:
    ...
    async def test_successful_extraction(self):
        ...
        result = await extract_wisdom_from_scene(
            scene_data={"start_time": 10.0, "end_time": 45.0, "scene_type": "interview"},
            actions=[{"action_type": "verbal", "start_time": 10.0, "end_time": 45.0, "intensity": 0.2}],
